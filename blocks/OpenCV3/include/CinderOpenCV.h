...
inline cv::Mat toOcv(ci::Surface8uRef sourceRef, int type = -1)
{
	if (type == -1) {
		int depth = CV_8U;
		int channels = sourceRef->getChannelOrder().getImageIoChannelOrder();
		type = CV_MAKETYPE(depth, channels);
	}

	cv::Mat result(sourceRef->getHeight(), sourceRef->getWidth(), type, sourceRef->getData());
	return result;
}
