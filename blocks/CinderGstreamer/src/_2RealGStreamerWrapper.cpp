...
bool GStreamerWrapper::open(std::string strFilename, bool bGenerateVideoBuffer, bool bGenerateAudioBuffer, unsigned int iBytesPerPixel)
{
	...
	if ( bGenerateVideoBuffer )
	{
		...
		GstCaps* caps = gst_caps_new_simple( "video/x-raw",
			...
			NULL );
		//GstCaps* caps = gst_caps_from_string("video/x-raw,format=RGBA,pixel-aspect-ratio=1/1,bpp=32,depth=24");
		...
		m_GstVideoSinkCallbacks.new_sample = &GStreamerWrapper::onNewSampleFromVideoSource;
	...
	if ( bGenerateAudioBuffer )
	{
		...
		m_GstAudioSinkCallbacks.new_sample = &GStreamerWrapper::onNewSampleFromAudioSource;
...
void GStreamerWrapper::close( bool joinThread )
{
	m_bFileIsOpen = false;
	m_CurrentPlayState = NOT_INITIALIZED;
...
// get rid of message handler thread
	...
	{
		// comment this laine out because it sometimes causes crash
	...
	{
		// comment this line out because it sometimes causes crash
...
{
	return getCurrentTimeInNs() / 1000000.0;
}
...
void GStreamerWrapper::retrieveVideoInfo()
{
	...
	if ( m_iNumVideoStreams > 0 )
	{
		...
		GstCaps* gstCaps = gst_pad_get_current_caps(gstPad);
		...
		if (gstPad) {
			if (gstCaps)
			{
				...
				GstVideoInfo info;
				gst_video_info_from_caps(&info, gstCaps);
				m_iWidth = info.width;
				m_iHeight = info.height;

				// Frame Rate
				//const GValue* framerate = gst_video_frame_rate(gstPad);
				int iFpsNumerator = info.fps_n; //gst_value_get_fraction_numerator(framerate);
				int iFpsDenominator = info.fps_d; //gst_value_get_fraction_denominator(framerate);

				// Number of frames
				m_iNumberOfFrames = (gint)((float)(m_iDurationInNs / GST_SECOND) * (float)iFpsNumerator / (float)iFpsDenominator);

				// FPS
				m_fFps = (float)iFpsNumerator / (float)iFpsDenominator;

				gst_object_unref(gstCaps);
			}
...
GstFlowReturn GStreamerWrapper::onNewPrerollFromVideoSource( GstAppSink* appsink, void* listener )
{
	...
	GstSample* gstVideoSinkSample = gst_app_sink_pull_preroll(GST_APP_SINK(appsink));
	((GStreamerWrapper *)listener)->newVideoSinkPrerollCallback(gstVideoSinkSample);
	gst_sample_unref(gstVideoSinkSample);
...
{
	GstSample* gstAudioSinkSample = gst_app_sink_pull_preroll( GST_APP_SINK( appsink ) );
	((GStreamerWrapper *)listener)->newAudioSinkPrerollCallback(gstAudioSinkSample);
	gst_sample_unref(gstAudioSinkSample);

	return GST_FLOW_OK;
}

GstFlowReturn GStreamerWrapper::onNewSampleFromVideoSource(GstAppSink* appsink, void* listener)
{
	GstSample* gstVideoSinkSample = gst_app_sink_pull_sample( GST_APP_SINK( appsink ) );
	((GStreamerWrapper *)listener)->newVideoSinkSampleCallback(gstVideoSinkSample);
	gst_sample_unref( gstVideoSinkSample );

	return GST_FLOW_OK;
}

GstFlowReturn GStreamerWrapper::onNewSampleFromAudioSource(GstAppSink* appsink, void* listener)
{
	GstSample* gstAudioSinkSample = gst_app_sink_pull_sample( GST_APP_SINK( appsink ) );
	((GStreamerWrapper *)listener)->newAudioSinkSampleCallback(gstAudioSinkSample);
	gst_sample_unref( gstAudioSinkSample );
...
{
	GstBuffer* videoSinkBuffer = gst_sample_get_buffer(videoSinkSample);
	...
	if ( m_cVideoBuffer == NULL )
	{
		...
		gint64 videoBufferSize = gst_buffer_get_size( videoSinkBuffer );
		m_cVideoBuffer = new unsigned char[(unsigned int) videoBufferSize];
	}

	// Copy the video appsink buffer data to our unsigned char array
	//memcpy( (unsigned char *)m_cVideoBuffer, (unsigned char *)GST_BUFFER_DATA( videoSinkBuffer ), gst_buffer_get_size( videoSinkBuffer ) );
	GstMapInfo bufferData;
	gboolean b = gst_buffer_map(videoSinkBuffer, &bufferData, GST_MAP_READ);
	memcpy((unsigned char *)m_cVideoBuffer, (unsigned char *)bufferData.data, gst_buffer_get_size(videoSinkBuffer));
	gst_buffer_unmap(videoSinkBuffer, &bufferData);
}

void GStreamerWrapper::newVideoSinkSampleCallback( GstSample* videoSinkSample)
{
	GstBuffer* videoSinkBuffer = gst_sample_get_buffer(videoSinkSample);
	...
	//memcpy( (unsigned char *)m_cVideoBuffer, (unsigned char *)GST_BUFFER_DATA( videoSinkBuffer ), gst_buffer_get_size( videoSinkBuffer ) );
	GstMapInfo bufferData;
	gst_buffer_map(videoSinkBuffer, &bufferData, GST_MAP_READ);
	memcpy((unsigned char *)m_cVideoBuffer, (unsigned char *)bufferData.data, gst_buffer_get_size(videoSinkBuffer));
	gst_buffer_unmap(videoSinkBuffer, &bufferData);
}
...
void GStreamerWrapper::newAudioSinkPrerollCallback( GstSample* audioSinkSample )
{
	GstBuffer* audioSinkBuffer = gst_sample_get_buffer(audioSinkSample);

	if ( m_cAudioBuffer == NULL )
	{
		m_iAudioBufferSize = gst_buffer_get_size( audioSinkBuffer );
		...
		*/
		...
		GstCaps* audioCaps = gst_sample_get_caps(audioSinkSample);
		...
		m_bIsAudioSigned = isAudioSigned ? true : false;
	...
	else
	{
		...
		int bufferSize = gst_buffer_get_size( audioSinkBuffer );
	...
	//memcpy( (unsigned char *)m_cAudioBuffer, (unsigned char *)GST_BUFFER_DATA( audioSinkBuffer ), gst_buffer_get_size( audioSinkBuffer ) );
	GstMapInfo bufferData;
	gst_buffer_map(audioSinkBuffer, &bufferData, GST_MAP_READ);
	memcpy((unsigned char *)m_cAudioBuffer, (unsigned char *)bufferData.data, gst_buffer_get_size(audioSinkBuffer));
	gst_buffer_unmap(audioSinkBuffer, &bufferData);
}

void GStreamerWrapper::newAudioSinkSampleCallback(GstSample* audioSinkSample)
{
	GstBuffer* audioSinkBuffer = gst_sample_get_buffer(audioSinkSample);
